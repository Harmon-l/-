...
def parse_args():
    ...
    parser.add_argument('--work-dir', type=str, help='path for get training config and saving log/models')
    parser.add_argument('--local_rank', type=int, default=0, help='LOCAL_PROCESS_RANK in DistributedDataParallel model')
...
def main():
    ...
    for epoch in range(start_epoch, config.epochs + 1):
        ...
        writer.add_scalar('acc1', top1, global_step=epoch)
        writer.add_scalar('acc5', top5, global_step=epoch)
        ...
        if (config.distributed and local_rank == 0) or not config.distributed:
            ...
            if top1 > best_top1:
                torch.save(model.module.state_dict(),
                           os.path.join(checkpoint_dir, '{}_best.pth'.format(config.data_augment)))
                best_top1 = top1
